...
//API TO get Table data for a selected month
app.get('/tabledata',validateMonth, async(req,res)=>{
  try{
    const { month } = req.query;
  ...
  `;
  const resultAllTransactions = await database.all(allTransactionsQuery, [monthNumber]);
  res.json(resultAllTransactions);
}catch (error) {
